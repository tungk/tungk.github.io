+++
title = 'Teaching'
draft = false
+++

<!-- # Teaching -->
## Coordination
- Software 7 (SW9).
- Software 9 (SW9).

## Courses
- Data Structures and Algorithms (shared with Prof. Bin Yang and Prof. Chenjuan Guo).
- Advanced Algorithms (shared with Prof. Bin Yang and Prof. Chenjuan Guo).
- Algorithms and Computability (shared with Prof. Bin Yang, Prof. Dalin Zhang, and Prof. Alvaro Torralba).
- Algorithms and Satisfiability (shared with Prof. Bin Yang, Prof. Dalin Zhang, and Prof. Alvaro Torralba).
- Introduction to Database Systems (at RMIT University).
- Programming Bootcamp (at RMIT University).
- Programming Studio (at RMIT University).
- Data-intensive Sytems.
- Specilization Course in Database/Data Mining.
- Mobile Data and Location-based Services (shared with Prof. Tiantian Liu).

## Supervision
- Software 3 (SW3).
- Software 4 (SW4).
- Software 5 (SW5).
- Bachelor's Thesis.
- Software 7 (SW7).
- Software 8 (SW8).
- Software 9 (SW9).
- Master's Thesis.

## Teaching Assistant
- Algorithms and Data Structures (shared with Prof. Sean Bin Yang).
- Advanced Algorithms (shared with Prof. Sean Bin Yang).
- Algorithms and Computability (shared with Prof. Jákup Odssonur Svöðstein).
- Algorithms and Satisfiability (shared with Prof. Jákup Odssonur Svöðstein).

# Mentees
- Cong-Dat Nguyen (at FPT AI Center).
- Tuan-Binh Tran (at Vin University).

# Bachelor Students
- Philip Shaffer-Lassen.
- Magnus Emil Wichmann.
- Jakob Rossander Kristensen.
- August Høivang Grane.
- Khanh-Toan Nguyen (at University of Science - Vietnam National Univeristy).
- Duc Kieu (at University of Science - Vietnam National Univeristy).

## Master Students
- David Campos (co-supervised with Prof. Bin Yang).
- Mik Christensen (co-supervised with Prof. Bin Yang).
- Thor Christian Koustrup-Müller.
- Benjamin Villadsen.
- Daniel Brenet.
- Benjamin Speyer Larsen.
- Muad Naim Ghanim.
- Simon Nagarkoti.
- Bikash Gurung.
- Nipa Roy.

## Ph.D. Students
- David Campos (co-supervised with Prof. Bin Yang).
- Jichen Yuan (co-supervised with Prof. Danh Le Phuoc).